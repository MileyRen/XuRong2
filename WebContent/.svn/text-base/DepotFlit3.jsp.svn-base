...
function checkform()
{
	if(document.getElementById("employeeName4").value == "") {
		alert("请选择一个调拨对象。");
		return false;
	}
 ...
 <input id="results" name="results" type="hidden"/>
  ...
  <table  style="line-height:20px;">
    ...
    <td style="vertical-align: middle; width:30%">
     <div style="width:auto; margin:5px; padding:0px" align="middle">
	...
	<input type="button" value="查询" onclick="doAjax()"  />
     ...
     <select size="15" multiple="multiple" name="srcid" id="selectsrcid" style="width:200px">
...
<table id="table">
	<tr>
		<td>
			<select name="employeeName3" id="employeeName3" >	  	
			  	<option value="">请选择</option>
			  	<c:forEach items="${users}" var="u" >
					<option value="${u.employeeName}">${u.employeeName}</option>
				</c:forEach>			
		...
		<td>
			<input type="button" value="查询" onclick="doAjax2()"/>
		</td>
		
		<td>
			<select name="employeeName4" id="employeeName4" >	  	
			  	<option value="">请选择</option>
			  	<c:forEach items="${users}" var="u" >
					<option value="${u.employeeName}">${u.employeeName}</option>
				</c:forEach>			
...
<div align="center">
    ...
    &nbsp;&nbsp;&nbsp;&nbsp;
    <input type="button" value="返回" onclick="location.href='<c:url value="/to_brand_page_3"/>'" class="button"/>
</div>
</form>
</div>
</div>
